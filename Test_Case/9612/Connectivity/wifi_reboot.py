...
import pytest
from Common.Globe_Api.Wifi_Page import DriverPage
...
class Test_wifi_reboot():
    ...
    def test_wifi_reboot(self, num, init_driver, init_data):
        driver = init_driver
        do_log,adb,picture_path,error_path,record_path,tombstones_path,anr_path,do_logcat,device = init_data
        do_log.info("第 " + str(num) + " 次" + "测试程序启动")

        do_logcat.open('第' + str(1) + '轮wifi_reboot')


        # 断言
        do_logcat.close()
        assert wifi_ping, "网络ping不通"
        assert wifi_text, "wifi显示未连接"
        do_log.info('第{}轮测试成功'.format(num))
        os.remove(do_logcat.logpath)
        # 开始重启设备
        adb.reboot()
